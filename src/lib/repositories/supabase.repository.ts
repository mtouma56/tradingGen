...
export class SupabaseRepository implements DataRepository {
  ...
  async createOperation(operation: Omit<Operation, 'id'>): Promise<Operation> {
    ...
    const { data, error } = await supabase
      .from('operations')
      .insert([{ ...rest, op_date: date_operation }])
      .select()
      .single();

    if (error) throw error;

    const { op_date, ...op } = data;
    return {
      ...op,
      date_operation: new Date(op_date)
    };
  ...
  async updateOperation(id: string, operation: Partial<Operation>): Promise<Operation> {
    ...
    const { date_operation, ...rest } = operation;
    const payload = date_operation ? { ...rest, op_date: date_operation } : rest;
  ...
  // Mouvements stock
    async getMouvements(
      filtres?: { type?: string; produit?: string; depot_id?: string }
